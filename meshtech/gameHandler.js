...
const setWcgTurnTimeout = (chatJid, MeshTech, currentPlayer, game) => {
    ...
    const timeout = setTimeout(async () => {
        ...
        if (activeGame && activeGame.currentTurn === currentPlayer) {
            ...
            if (result.finished) {
                ...
                setWcgTurnTimeout(chatJid, MeshTech, result.nextPlayer, activeGame);
            }
        }
...
const setDiceTurnTimeout = (chatJid, MeshTech, currentPlayer, game) => {
...
async function handleAiWcgMove(from, MeshTech, gameRef) {
...
async function handleAiDiceRoll(from, MeshTech, gameRef) {
    ...
    if (result.gameFinished) {
        ...
        setDiceTurnTimeout(from, MeshTech, result.player1, freshGame);
    }
...
const handleGameMessage = async (MeshTech, message) => {
    try {
        ...
        if (lowerBody === 'roll') {
            ...
            if (result.roundComplete || result.gameFinished) {
                ...
                if (game.isAiGame && result.waitingFor === BOT_JID) {
                    ...
                    await handleAiDiceRoll(from, MeshTech, game);
                    return;
                }
        ...
        if (wcgGame) {
            ...
            if (updatedGame && updatedGame.isAiGame && result.nextPlayer === BOT_JID) {
                ...
                await handleAiWcgMove(from, MeshTech, updatedGame);
                return;
            }
