...
async function loadSession() {
    try {
        ...
        const v22Match = sessionId.match(/^MESH-TECH-MD:~(.+)$/i);
        const legacyMatch = sessionId.match(/^MeshTech~(.+)$/i);
        let credentials;

        if (v22Match?.[1]) {
            // V2.2 format: raw creds.json bytes encoded directly as Base64.
            credentials = Buffer.from(v22Match[1], 'base64');
        } else if (legacyMatch?.[1]) {
            // V2.5 transitional format: gzip-compressed creds.json in Base64.
            const compressedData = Buffer.from(legacyMatch[1].replace('...', ''), 'base64');
            credentials = zlib.gunzipSync(compressedData);
        } else {
            throw new Error("❌ Invalid session format. Expected 'MESH-TECH-MD:~.....'");
        }

        if (!credentials.length) {
            throw new Error("❌ Empty session credentials. Generate a fresh session ID after pairing.");
        }

        try {
            JSON.parse(credentials.toString('utf8'));
        } catch (_) {
            throw new Error("❌ Invalid session credentials. Generate a fresh session ID after pairing.");
        }
        ...
        fs.writeFileSync(sessionPath, credentials, "utf8");
...
function verifyJidState(jid) {
    ...
    console.log('Welcome to MESH TECH MD', jid);
    return true;
}
